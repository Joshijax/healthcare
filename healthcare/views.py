...
from .preg import function

def index(request):
    return render(request, 'healthcare/home.html')

def advice(request):
    return render(request, 'healthcare/advice.html')
...
def BMR(request):
    ...
    if gender == 'male':
        ...
        bmr = 9.6 * int(weight) + 1.8 * int(height) - 4.7 * int(age) + 655
    else:
        bmr = -1
...
def PRG(request):
    ...
    monthh = request.POST.get('height')
    dayy= request.POST.get('weight')
    print(monthh, dayy)
    
    context = {'WHR': function(monthh, int(dayy))}
    return JsonResponse(context)
